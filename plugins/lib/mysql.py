#!/usr/bin/env python
# encoding: utf-8

import MySQLdb
import time

class MySQL:
    dbconfig = {'host':'localhost',
                'port': 3306,
                'user':'root',
                'passwd':'',
                'db':'wyproxy',
                'charset':'utf8'}
    error_code = '' #MySQL错误号码

    _instance = None #本类的实例
    _conn = None # 数据库conn
    _cur = None #游标

    _TIMEOUT = 30 #默认超时30秒
    _timecount = 0
        
    def __init__(self):
        try:
            self._conn = MySQLdb.connect(host=self.dbconfig['host'],
                                         port=self.dbconfig['port'], 
                                         user=self.dbconfig['user'],
                                         passwd=self.dbconfig['passwd'],
                                         db=self.dbconfig['db'],
                                         charset=self.dbconfig['charset'])
        except MySQLdb.Error, e:
            self.error_code = e.args[0]
            error_msg = 'MySQL error! ', e.args[0], e.args[1]
            print error_msg
            
            # 如果没有超过预设超时时间，则再次尝试连接，
            if self._timecount < self._TIMEOUT:
                interval = 5
                self._timecount += interval
                time.sleep(interval)
                return self.__init__(dbconfig)
            else:
                raise Exception(error_msg)
        
        self._cur = self._conn.cursor(cursorclass=MySQLdb.cursors.DictCursor)
        self._instance = MySQLdb

    def query(self,sql,value=None):
        u'执行 SELECT 语句'  
        try:
            self._cur.execute("SET NAMES utf8") 
            if value == None:
                result = self._cur.execute(sql)
            else:
                result = self._cur.execute(sql,value)
        except MySQLdb.Error, e:
            self.error_code = e.args[0]
            print "数据库错误代码:",e.args[0],e.args[1]
            result = False
        return result

    def update(self,sql,value=None):
        u'执行 UPDATE 及 DELETE 语句'
        try:
            self._cur.execute("SET NAMES utf8") 
            if value == None:
                result = self._cur.execute(sql)
            else:
                result = self._cur.execute(sql,value)
            self._conn.commit()
        except MySQLdb.Error, e:
            self.error_code = e.args[0]
            print "数据库错误代码:",e.args[0],e.args[1]
            result = False
        return result
        
    def insert(self,sql,value=None):
        u'执行 INSERT 语句。如主键为自增长int，则返回新生成的ID'
        try:
            self._cur.execute("SET NAMES utf8")
            if value == None:
                result = self._cur.execute(sql)
            else:
                result = self._cur.execute(sql,value)
            self._conn.commit()
            return self._conn.insert_id()
        except MySQLdb.Error, e:
            self.error_code = e.args[0]
            return False
    def insert_pl(self,sql,value):
        try:
            self._cur.execute("SET NAMES utf8")
            result = self._cur.executemany(sql,value)
            self._conn.commit()
        except MySQLdb.Error, e:
            self.error_code = e.args[0]
            return False
    
    def fetchAllRows(self):
        u'返回结果列表'
        return self._cur.fetchall()
 
    def getRowCount(self):
        u'获取结果行数'
        return self._cur.rowcount
                          
    def getCount(self, result):
        u'快速获取结果行数'
        result = result[0]
        key = result.keys()[0]
        return result[key]
    def commit(self):
        u'数据库commit操作'
        self._conn.commit()
           
    def __del__(self): 
        u'释放资源（系统GC自动调用）'
        try:
            self._cur.close() 
            self._conn.close() 
        except:
            pass
        
    def  close(self):
        u'关闭数据库连接'
        self.__del__()
 

if __name__ == '__main__':
    #连接数据库，创建这个类的实例
    db = MySQL()
    
    #操作数据库
    sql = "SELECT 1 FROM dual where '1'=%s"
    value = [1]
    db.query(sql,value)
    
    #获取结果列表
    
    result = db.fetchAllRows()
    #相当于php里面的var_dump
    print result
    
    #对行进行循环
    for row in result:
        #对列进行循环
        for colum in row:
            print colum
 
    #关闭数据库
    db.close()
