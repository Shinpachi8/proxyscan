...
def verify(task):
    ...
    # if url_parse.query == "":
    #     for key in USR_Rule.keys():
    #         for payload in USR_Rule[key]:
    #             _ = copy.deepcopy(task)
    #             tmp_url = url + payload
    #             _["url"] = tmp_url
    #             urlQueue.put(_)
    ...
    #         for payload_key in USR_Rule.keys():
    #             for payload in USR_Rule[payload_key]:
    #                 # lfi时选择直接替换参数值
    #                 tmp_query = ""
    ...
    #     try:
    #         logger.info("[FuzzUSR][verify]: {}".format(url))
    #         if item["method"] == "GET":
    ...
    #         if re.findall(anchor, response) and req.status_code == 200:
